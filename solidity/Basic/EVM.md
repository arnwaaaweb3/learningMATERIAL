...
1. **You're sending some amount of tokens to someone (transfer).**  
   ...
   - `value`: the amount of ETH to be sent  
     ...
     - This is used if you want to transfer some ETH to the receiver, or if you want to pay a smart contract some ETH to do something.  
     - Example:  
       ```json
       {
           "nonce": 12,
...
2. **Signing & Broadcasting**  
   Once the transaction object is created, here's what's gonna happen:  
   - **Signing**:  
     Your wallet (Metamask/Rabby) uses your Private Key to sign this transaction.  
     This proves that YOU (the owner of the address) authorized this specific action.  
   - **Broadcasting**: The signed transaction is sent to the "Mempool" (Memory Pool). Think of this as a waiting room for pending transactions.  
